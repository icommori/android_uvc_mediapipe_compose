...
class FPSCounter(val onFPS: (fps:Int)-> Unit){
    var count: Int = 0
    var mytime: Long = 0
    var lastFPS:Int=0
    fun count(){
        if (SystemClock.uptimeMillis() - mytime >= 1000) {
            mytime = SystemClock.uptimeMillis()
            lastFPS = count
            onFPS(count)
            //Log.v("innocomm","FPSCounter "+this+" "+count)
            count = 0
        } else {
            count++
        }
...
class ObjectDetectorHelper(
    ...
    //innocomm
    var oKForNext = true
    var previewFPS = FPSCounter {
        objectDetectorListener?.onFPS(it)
    }
    var detectFPS = FPSCounter {}
    ...
    fun detectVideoFile(
        ...
        Log.v("innocomm","detectVideoFile "+videoUri.toString())
        ...
        return if (didErrorOccurred) {
            ...
            ResultBundle(resultList, inferenceTimePerFrameMs, height, width,1)
        }
    }
    ...
    fun detectLivestreamFrame2(data: ByteArray) {
        previewFPS.count()
        if(!oKForNext){
            //Log.v("innocomm","drop frame "+data)
            return
        }
        detectFPS.count()
        oKForNext = false
        ...
        val yuvImage =
            YuvImage(data, ImageFormat.NV21, myApp.previewSize.width, myApp.previewSize.height, null)
        ByteArrayOutputStream().use { outStream ->
            ...
            val bitmapBuffer = BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.size)
            bitmapBuffer?.let {
    ...
    companion object {
        ...
        const val CAMSTATE_IDLE = 0
        const val CAMSTATE_INITIALIZING = 1
        const val CAMSTATE_PREVIEWING = 2
